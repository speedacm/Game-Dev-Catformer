...
@export var vel_multiplier := 3
@export var vel_length_max := 4000
	...
	if player.on_floor():
		...
		if Input.is_action_just_released("Click"):
			...
			if vel.length() > vel_length_max:
				player.apply_impulse(vel.normalized() * vel_length_max) # fix this rate of slow down (way too harsh rn)
				
			else:
					player.apply_impulse(vel) 
			print("player velocity:" , player.linear_velocity)
			hide()
		hide()
